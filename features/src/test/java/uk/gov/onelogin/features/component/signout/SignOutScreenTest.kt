package uk.gov.onelogin.features.component.signout
...
import uk.gov.onelogin.features.signout.ui.SignOutAnalyticsViewModel
...
class SignOutScreenTest : FragmentActivityTestCase() {
    ...
    fun verifyScreenDisplayed() {
        ...
        verify(analytics).logEventV3Dot1(
            SignOutAnalyticsViewModel.Companion.makeSignOutViewEvent(
                context
            )
        )
    }
    ...
    fun verifySignOutButtonSucceeds() =
        runBlocking {
            ...
            verify(analytics).logEventV3Dot1(
                SignOutAnalyticsViewModel.Companion.onPrimaryEvent(
                    context
                )
            )
    ...
    fun verifyCloseIconButton() {
        ...
        verify(analytics).logEventV3Dot1(SignOutAnalyticsViewModel.Companion.onCloseIcon())
