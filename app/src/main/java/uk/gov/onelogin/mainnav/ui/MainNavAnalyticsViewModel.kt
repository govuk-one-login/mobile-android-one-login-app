...
@HiltViewModel
class MainNavAnalyticsViewModel
    @Inject
    constructor(
        ...
        companion object {
            internal fun makeHomeButtonEvent(context: Context) =
                with(context) {
                    TrackEvent.Icon(
            ...
            internal fun makeWalletButtonEvent(context: Context) =
                with(context) {
                    TrackEvent.Icon(
            ...
            internal fun makeSettingsButtonEvent(context: Context) =
                with(context) {
                    TrackEvent.Icon(
                        text = getEnglishString(R.string.app_settings),
                        params = getRequiredParams(TaxonomyLevel2.SETTINGS)
                    )
                }

            private fun getRequiredParams(taxonomyL2: TaxonomyLevel2) =
                RequiredParameters(
