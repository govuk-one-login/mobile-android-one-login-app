...
class ResultCollectionUtilTest {
    ...
    @Test
    fun `runConcurrentlyForResults Exception handling`() = runTest {
        // Given a ResultCollectionUtil
        val results = mutableListOf<String>()
        val suspendFunctions = listOf<suspend () -> Result<Unit>>(
            { results.add("Function 0 complete"); Result.success(Unit) },
            { throw IllegalStateException() },
            { results.add("Function 2 complete"); Result.success(Unit) }
        )
    ...
    @Test
    fun `runConcurrentlyForResults runs concurrently`() = runTest {
        // Given a ResultCollectionUtil
        val delayTime = 1000L
        val suspendFunctions = listOf<suspend () -> Result<Unit>>(
            { delay(delayTime); Result.success(Unit) },
            { delay(delayTime); Result.success(Unit) },
            { delay(delayTime); Result.success(Unit) },
            { delay(delayTime); Result.success(Unit) }
        )
        // When calling runConcurrentlyForResults
        val totalTime = with(util) {
            measureTime {
                suspendFunctions.runConcurrentlyForResults()
            }
        }
